...
const TENANT_MODELS = [
  'Transaction', 'Customer', 'User', 'Payment', 'Remittance',
  'P2POrder', 'Account', 'BankAccount', 'Invoice', 'Receipt',
  'Document', 'AuditLog', 'Notification', 'Subscription',
  'SalesPlan', 'Plan', 'TenantSettings', 'ExchangeRate',
  'CryptoOrder', 'CryptoTransaction', 'CurrencyTransaction',
  'InterBranchTransfer', 'Debt', 'Discrepancy', 'JournalEntry'
];
...
class DatabaseQueryAuditor {
  ...
  async scanFiles() {
    ...
    console.log(`🔍 Scanning ${files.length} files for database query issues...\n`);
      ...
      if (matches) {
        matches.forEach(match => {
          ...
          this.issues.push({
            ...
            recommendation: 'Add tenant_id filter to prevent data leakage across tenants'
          });
      ...
      if (matches) {
        matches.forEach(match => {
          ...
          this.issues.push({
            ...
            recommendation: 'Use findOne with tenant_id filter instead of findById'
          });
          this.stats.securityIssues++;
        });
      }
  ...
  checkMissingIndexes(filePath, content, lines) {
    ...
    if (filePath.includes('models/')) {
      ...
      if (TENANT_MODELS.includes(modelName)) {
        if (!content.includes('.index({ tenant_id: 1 })') && 
          ...
          this.issues.push({
            ...
            recommendation: 'Add index on tenant_id field for better query performance'
          });
  ...
  generateReport() {
    ...
    if (this.issues.length === 0) {
      console.log('✅ No issues found! All database queries are properly secured and optimized.');
      return;
    }
    ...
    console.log('============================');
    console.log('1. Always include tenant_id in WHERE clauses for multi-tenant data');
    console.log('2. Use compound indexes on (tenant_id, other_fields) for better performance');
    console.log('3. Avoid findById() - use findOne({ _id: id, tenant_id: tenantId }) instead');
    console.log('4. Implement query middleware to automatically add tenant filters');
    console.log('5. Use database connection pooling for better performance');
    console.log('6. Consider implementing row-level security at the database level');
    console.log('7. Regular audit of database queries for security compliance\n');
