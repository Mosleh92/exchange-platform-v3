...
class SmokeTests {
  ...
  async testHealthEndpoint() {
    await this.test("Health Endpoint", async () => {
      ...
      this.assert(
        ...
        "Health response should include components",
      );
      this.assert(
        response.data.components.database.status === "healthy",
        "Database should be healthy",
      );
      this.assert(
        response.data.components.redis.status === "healthy",
  ...
  async testAuthentication() {
    await this.test("Authentication Flow", async () => {
      ...
      this.assert(
        ...
        "Profile response should include user data",
      );
  ...
  async testTenantIsolation() {
    await this.test("Tenant Isolation", async () => {
      ...
      const attemptResponse = await this.client.get(
        `/api/tenants/${wrongTenantId}/users`,
        {
          headers: { Authorization: `Bearer ${this.authToken}` },
        },
      );
      ...
      if (currentUser.tenantId) {
        const ownTenantResponse = await this.client.get(
          `/api/tenants/${currentUser.tenantId}/users`,
          {
            headers: { Authorization: `Bearer ${this.authToken}` },
          },
        );
        ...
        this.assert(
          ownTenantResponse.status === 200,
          "Should allow access to own tenant data",
        );
      }
    });
  }
  ...
  async testBasicCRUD() {
    await this.test("Basic CRUD Operations", async () => {
      ...
      this.assert(
        ...
        "Should be able to read transactions",
      );
      this.assert(
      ...
      if (usersResponse.status === 200) {
        ...
        this.assert(
          ...
          "Non-admin users should get 403 for user list",
        );
      }
    });
  }
  ...
  async testPaymentFlow() {
    await this.test("Payment System", async () => {
      ...
      this.assert(
        methodsResponse.status === 200,
        "Should be able to get payment methods",
      );
      ...
      this.assert(
        ...
        "Should be able to get payment history",
      );
      this.assert(
        Array.isArray(historyResponse.data.data.payments),
  ...
  async testReporting() {
    await this.test("Reporting System", async () => {
      ...
      this.assert(
        ...
        "Should be able to get dashboard stats",
      );
      this.assert(
        statsResponse.data.data.statistics,
        "Stats response should include statistics",
      );

      // Test recent transactions
      const recentResponse = await this.client.get(
      ...
      this.assert(
        recentResponse.status === 200,
        "Should be able to get recent transactions",
      );
      this.assert(
        Array.isArray(recentResponse.data.data.transactions),
        "Recent transactions should be an array",
      );
    });
  }
...
program
  ...
  .option(
    "-e, --env <environment>",
    "Environment to test (staging|production)",
    "staging",
  )
