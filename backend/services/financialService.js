...
class FinancialService {
    ...
    async updateBalance(customerId, currency, amount, transactionType, metadata = {}) {
        ...
        try {
            ...
            if (amount < 0) {
                ...
                if (currentBalance + amount < 0) {
                    throw new Error(`Insufficient balance. Current: ${currentBalance}, Requested: ${Math.abs(amount)}`);
                }
            }
            ...
            await this.logFinancialOperation('BALANCE_UPDATE', {
                customerId,
                currency,
                amount,
                transactionType,
    ...
    async processExchange(customerId, fromCurrency, toCurrency, amount, exchangeRate, metadata = {}) {
        const session = await mongoose.startSession();
        session.startTransaction();
        
        try {
            // Validate exchange rate (prevent manipulation)
            const validRate = await this.validateExchangeRate(fromCurrency, toCurrency, exchangeRate);
            if (!validRate) {
            ...
            // Debit from currency
            await this.updateBalance(customerId, fromCurrency, -amount, 'EXCHANGE_DEBIT', {
                ...metadata,
                toCurrency,
                exchangeRate,
                convertedAmount
            ...
            // Credit to currency
            await this.updateBalance(customerId, toCurrency, convertedAmount, 'EXCHANGE_CREDIT', {
                ...metadata,
                fromCurrency,
                exchangeRate,
    ...
    async validateExchangeRate(fromCurrency, toCurrency, providedRate) {
        ...
        const rateAge = (Date.now() - latestRate.createdAt.getTime()) / (1000 * 60 * 60);
        if (rateAge > 1) {
