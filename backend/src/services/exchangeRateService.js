...
class ExchangeRateService {
  ...
  async getApplicableRate(fromCurrency, toCurrency, transactionType) {
    ...
    if (
      (fromCurrency === "IRR" && toCurrency === "AED") ||
      transactionType === "buy_aed" ||
      transactionType === "exchange_irr_aed"
    ) {
      ...
      (fromCurrency === "AED" && toCurrency === "IRR") ||
      transactionType === "sell_aed" ||
      transactionType === "exchange_aed_irr"
    ) {
    ...
    logger.warn(
      `No applicable rate found for transaction: ${fromCurrency} -> ${toCurrency}, Type: ${transactionType}`,
    );
  ...
  async updateSpecificRate(pair, rateType, value) {
    ...
    logger.error(
      `Failed to update rate for ${pair} (${rateType}). Invalid pair or rate type.`,
    );
    return false;
  }
}
