...
class AccountService {
  ...
  static async createAccount(tenantId, userId, accountData) {
    try {
      ...
      if (error.code === 11000) {
        throw new Error(
          "شماره حساب یا نام حساب برای این سازمان قبلاً ثبت شده است.",
        );
      }
  ...
  static async getAccount(tenantId, accountId) {
    try {
      ...
      logger.error("Error fetching account by ID:", {
        error: error.message,
        tenantId,
        accountId,
      });
  ...
  static async getAccounts(tenantId, query = {}) {
    try {
      ...
      if (query.account_name)
        filter.account_name = { $regex: query.account_name, $options: "i" };
  ...
  static async updateBalance(
    ...
    try {
      ...
      if (newBalance < 0 && balanceType === "available") {
        ...
        throw new Error(
          `موجودی ${balanceType} نمی‌تواند منفی شود برای ارز ${currency}.`,
        );
      }
      ...
      logger.info(
        `Balance updated: ${balanceType} for account ${accountId} in ${currency}`,
        { amount, newBalance: currentBalance[balanceType] },
      );
      return account;
    } catch (error) {
