...
class AccountingService {
  ...
  static async calculateTransactionAmounts(transaction) {
    try {
      ...
      if (type === "buy_aed" || type === "sell_aed" || type === "exchange") {
        rate = await ExchangeRateService.getApplicableRate(
          currency_from,
          currency_to,
          type,
        );
      ...
      return {
        ...
        receivedIRR:
          currency_to === "IRR" && type !== "withdraw_irr"
            ? calculatedAmountTo
            : currency_from === "IRR" && type !== "withdraw_irr"
              ? amount_from
              : 0,
        receivedAED:
          currency_to === "AED" && type !== "withdraw_aed"
            ? calculatedAmountTo
            : currency_from === "AED" && type !== "withdraw_aed"
              ? amount_from
              : 0,
      ...
      logger.error("Transaction calculation error:", {
  ...
  static calculateBalances(transactions) {
    try {
      ...
      const unconverted = transactions.reduce(
        (acc, t) => ({
          aedUnconverted:
            acc.aedUnconverted +
  ...
  static validateTransaction(transaction) {
    ...
    switch (transaction.type) {
      ...
      case "exchange":
        if (
          !transaction.exchangeRateIRRAED ||
          !transaction.exchangeRateAEDIRR
        ) {
