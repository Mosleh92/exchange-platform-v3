...
class HoldManager {
  ...
  static async createHold(
    ...
    try {
      ...
      logger.error("Error creating hold:", {
        error: error.message,
        tenantId,
        accountId,
        currency,
  ...
  static async releaseHold(tenantId, holdTransactionId, releaseOptions = {}) {
    ...
    try {
      ...
      holdTransaction.metadata.releasedBy =
        releaseOptions.releasedBy || "system";
  ...
  static async getActiveHolds(tenantId, accountId, currency = null) {
    try {
      ...
      logger.error("Error fetching active holds:", {
        error: error.message,
        tenantId,
        accountId,
        currency,
