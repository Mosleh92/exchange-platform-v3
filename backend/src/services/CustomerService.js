    ...
    try {
      ...
      const defaultAccount = await AccountService.createAccount(
        ...
        userId,
        {
          ...
          balances: {
            // Initialize with zero balances for common currencies
            AED: {
              available: 0,
              onHold: 0,
      ...
      logger.info("Customer and default account created successfully", {
      ...
      logger.error("Error creating customer and account:", {
        error: error.message,
        tenantId,
        customerData,
      });
    ...
    try {
      ...
      if (customer) {
        ...
        if (accounts.length > 0) {
          ...
          logger.warn(
            "Existing customer found without an account, created a new default account.",
            { customerId: customer._id, accountId: account._id, tenantId },
          );
        }
        logger.info("Existing customer and account found.", {
        ...
        logger.info(
          "Customer not found, creating new customer and default account.",
          { tenantId, customerData },
        );
        const result = await this.createCustomer(
          tenantId,
          userId,
          customerData,
        ); // Use the existing createCustomer logic
        return result; // This already returns { customer, account }
      }
    } catch (error) {
      logger.error("Error in findOrCreateCustomerAndAccount:", {
        error: error.message,
        tenantId,
        customerData,
      });
      ...
      const customersWithAccounts = await Promise.all(
        customers.map(async (customer) => {
          ...
          accounts.forEach((account) => {
            customer.totalBalances[account.currency] =
              (customer.totalBalances[account.currency] || 0) +
