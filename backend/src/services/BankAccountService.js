...
class BankAccountService {
  ...
  static async getBankAccountById(tenantId, accountId) {
    try {
      ...
      if (!account) {
        logger.warn("Bank account not found or does not belong to tenant", {
          accountId,
          tenantId,
        });
        return null;
  ...
  static async updateBankAccount(tenantId, accountId, updateData) {
    try {
      ...
      if (!updatedAccount) {
        logger.warn(
          "Bank account not found for update or does not belong to tenant",
          { accountId, tenantId, updateData },
        );
        return null;
      }
  ...
  static async deleteBankAccount(tenantId, accountId) {
    try {
      ...
      if (result.deletedCount === 0) {
        logger.warn(
          "Bank account not found for deletion or does not belong to tenant",
          { accountId, tenantId },
        );
        return false;
      }
