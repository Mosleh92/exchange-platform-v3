...
class TransactionWorkflowService {
    ...
    try {
      ...
      const transaction = new Transaction({
        ...
        transaction_flow: {
          ...
          payment_destination: exchange_account
            ? "exchange_account"
            : "other_accounts",
        },
  ...
  static async addSplitPayments(transactionId, splitAccounts) {
    ...
    try {
      ...
      const totalSplitAmount = splitAccounts.reduce(
   ...
   * @returns {Promise<Object>} Updated transaction
   */
  static async uploadSplitPaymentReceipt(
    transactionId,
    accountNumber,
    receiptData,
  ) {
    ...
    try {
      ...
      // Send notification
      await NotificationService.sendPaymentReceiptUploadedNotification(
        transaction,
        splitPayment,
      );
  ...
  static async updateTransactionFlow(transactionId, newStatus, options = {}) {
    ...
    try {
      ...
      if (options.payment_destination) {
        transaction.transaction_flow.payment_destination =
          options.payment_destination;
      }
      ...
      // Send notification
      await NotificationService.sendTransactionStatusUpdateNotification(
        transaction,
      );
