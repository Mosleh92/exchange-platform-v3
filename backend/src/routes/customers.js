...
router.post(
  ...
  [
    ...
    body("phone")
      .notEmpty()
      .withMessage("شماره تلفن مشتری الزامی است")
      .trim()
    ...
    body("national_id")
      ...
      .withMessage("کد ملی باید 10 رقمی باشد"),
    body("address")
      .optional()
      .trim()
      .isLength({ max: 500 })
...
router.put(
  ...
  [
    ...
    body("status")
      .optional()
      .isIn(["pending", "verified", "rejected", "active", "inactive"])
      .withMessage("وضعیت مشتری نامعتبر است"),
    validateRequest,
  ],
  async (req, res) => {
    try {
  ...
  async (req, res) => {
    try {
      ...
      const { customer, account } =
        await CustomerService.findOrCreateCustomerAndAccount(tenantId, userId, {
          name,
          phone,
          email,
          national_id,
      ...
      logger.error("Error in findOrCreate customer route:", {
        error: error.message,
        tenantId: req.user.tenantId,
        body: req.body,
      });
...
router.get(
  "/my-special-rates",
  authorize(["customer"]),
  customerController.getMySpecialRates,
);
