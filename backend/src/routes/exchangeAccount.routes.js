...
// Get all accounts
router.get('/', authorize(['tenant_admin', 'branch_manager']), exchangeAccountController.getAllAccounts);

// Create new account
router.post('/', validateExchangeAccount, exchangeAccountController.createAccount);

// Get account by ID
router.get('/:accountId', exchangeAccountController.getAccountById);

// Get account by number
router.get('/number/:accountNumber', exchangeAccountController.getAccountByNumber);
...
// Calculate interest
router.post('/:accountId/calculate-interest', exchangeAccountController.calculateInterest);

// Close account
router.post('/:accountId/close', exchangeAccountController.closeAccount);

// Get customer accounts
router.get('/customer/:customerId', exchangeAccountController.getCustomerAccounts);
...
router.get('/my-accounts', authorize(['customer']), exchangeAccountController.getMyAccounts);

// فقط tenant_admin و branch_manager به لیست همه حساب‌ها دسترسی دارند
router.get('/all', authorize(['tenant_admin', 'branch_manager']), exchangeAccountController.getAllAccounts);
