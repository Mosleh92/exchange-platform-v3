...
const { validateP2POrder, validateP2PMessage, validateP2POffer } = require('../middleware/validation');
...
// P2P Chat Routes
router.post('/orders/:orderId/chat',
  authorize(['admin', 'manager', 'staff']),
...
router.post('/chats/:chatId/offers',
  authorize(['admin', 'manager', 'staff']),
  validateP2POffer,
  p2pController.makeOffer
);

router.put('/chats/:chatId/offers/:offerId/respond',
  authorize(['admin', 'manager', 'staff']),
  p2pController.respondToOffer
);

// ارسال پیشنهاد جدید در چت P2P
router.post('/chats/:chatId/send-offer',
  authorize(['admin', 'manager', 'staff']),
  p2pController.sendOfferInChat
);

// پاسخ به پیشنهاد (تایید یا رد) در چت P2P
router.post('/chats/:chatId/respond-offer',
  authorize(['admin', 'manager', 'staff']),
