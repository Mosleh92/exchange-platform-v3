...
router.get("/transactions", async (req, res) => {
  try {
    ...
    logger.error("Error fetching transactions for tenant:", {
      error: error.message,
...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      const customerAccounts = await AccountService.getAccounts(tenantId, {
...
router.get(
  ...
  async (req, res) => {
    try {
      ...
      balances.aedStatus = AccountingService.determineAccountStatus(
        balances.aedBalance,
      );
      balances.irrStatus = AccountingService.determineAccountStatus(
        balances.irrBalance,
      );
      ...
      logger.error("Balance calculation error:", {
        error: error.message,
        tenantId: req.user.tenantId,
