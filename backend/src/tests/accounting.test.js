...
describe('Data Isolation Security Test', () => {
  ...
  beforeAll(async () => {
    ...
    await mongoose.connect('mongodb://localhost:27017/exchange-platform-test', { useNewUrlParser: true, useUnifiedTopology: true });
...
describe('صحت ثبت اسناد حسابداری', () => {
  ...
  beforeAll(async () => {
    ...
    // ایجاد تراکنش تستی
    tx = await Transaction.create({ tenantId, branchId, type: 'deposit', amount: 5000 });
