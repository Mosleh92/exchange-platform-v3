...
describe("Tenant Isolation Integration Tests", () => {
  ...
  describe("User Management Isolation", () => {
    it("should only show users from same tenant", async () => {
      ...
      const userTenantIds = response.body.data.users.map(
        (user) => user.tenantId,
      );
    ...
    it("should allow super admin to access all users", async () => {
      ...
      const tenant1Users = users.filter(
        (user) => user.tenantId === tenant1._id.toString(),
      );
      const tenant2Users = users.filter(
        (user) => user.tenantId === tenant2._id.toString(),
      );
  ...
  describe("Transaction Isolation", () => {
    ...
    it("should only return transactions from same tenant", async () => {
      ...
      expect(response.body.data.transactions[0].tenantId).toBe(
        tenant1._id.toString(),
      );
      expect(response.body.data.transactions[0].id).toBe(
  ...
  describe("Account Isolation", () => {
    ...
    it("should only show accounts from same tenant", async () => {
      ...
      expect(response.body.data.accounts[0].tenantId).toBe(
        tenant1._id.toString(),
      );
    });
  ...
  describe("Branch Isolation", () => {
    it("should only show branches from same tenant", async () => {
      ...
      expect(response.body.data.branches[0].tenantId).toBe(
        tenant1._id.toString(),
      );
    });
  ...
  describe("Report Isolation", () => {
    ...
    it("should not include cross-tenant data in reports", async () => {
      ...
      expect(response1.body.data.report.totalAmount).not.toBe(
        response2.body.data.report.totalAmount,
      );
    });
  });
  ...
  describe("Database Query Isolation", () => {
    ...
    it("should automatically filter database queries by tenant", async () => {
      ...
      expect(response.body.data.customers[0].email).toBe(
