...
const token1 = jwt.sign({ _id: user1, tenantId: tenant1, branchId: branch1, role: 'tenant_admin', tenantAccess: [tenant1] }, 'testsecret');
const token2 = jwt.sign({ _id: user2, tenantId: tenant2, branchId: branch2, role: 'tenant_admin', tenantAccess: [tenant2] }, 'testsecret');
...
describe('ایزولاسیون داده بین tenantها', () => {
  ...
  beforeAll(async () => {
    ...
    tx1 = await Transaction.create({ tenantId: tenant1, branchId: branch1, type: 'payment', amount: 1000 });
    tx2 = await Transaction.create({ tenantId: tenant2, branchId: branch2, type: 'payment', amount: 2000 });
  });
  afterAll(async () => {
