...
describe('Customer Search & Filter', () => {
  ...
  beforeAll(async () => {
    ...
    await Customer.create([
      { name: 'Ali Test', phone: '+989111111111', email: 'ali@test.com', tenant_id: tenantId, kyc_status: 'verified' },
      { name: 'Sara Example', phone: '+989122222222', email: 'sara@example.com', tenant_id: tenantId, kyc_status: 'pending' },
      { name: 'Reza Filter', phone: '+989133333333', email: 'reza@filter.com', tenant_id: tenantId, kyc_status: 'rejected' }
    ]);
  ...
  it('should search customers by name', async () => {
    ...
    expect(res.body.data).toEqual(
      expect.arrayContaining([
        expect.objectContaining({ name: 'Ali Test' })
      ])
    );
  });
