...
// Indexes
exchangeRateSchema.index({ tenantId: 1, 'currencyPair.from': 1, 'currencyPair.to': 1 }, { unique: true });
...
exchangeRateSchema.statics.getCurrentRate = function(tenantId, fromCurrency, toCurrency) {
  const now = new Date();
  return this.findOne({
    tenantId,
...
exchangeRateSchema.statics.getVIPRate = function(tenantId, fromCurrency, toCurrency, vipLevel) {
  return this.getCurrentRate(tenantId, fromCurrency, toCurrency)
    .then(rate => {
