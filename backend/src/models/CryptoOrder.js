...
const cryptoOrderSchema = new mongoose.Schema({
  ...
  paymentMethod: { type: String, enum: ['bank_card', 'bank_transfer', 'cash', 'internal_wallet'], required: true },
  receiveMethod: { type: String, enum: ['wallet', 'bank_card', 'cash'], required: true },
  walletAddress: { type: String }, // آدرس کیف پول مقصد یا مبدا
  status: { type: String, enum: ['pending_payment', 'paid', 'under_review', 'confirmed', 'cancelled', 'rejected'], default: 'pending_payment' },
  receipt: {
    fileUrl: String,
