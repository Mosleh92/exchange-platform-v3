...
tenantSettingsSchema.methods.addToHistory = function (
  field,
  oldValue,
  newValue,
  changedBy,
...
tenantSettingsSchema.methods.updateExchangeRate = function (
  currency,
  rate,
  updatedBy,
) {
  const currencyIndex = this.exchangeRates.supportedCurrencies.findIndex(
    (c) => c.code === currency,
  );
  if (currencyIndex !== -1) {
