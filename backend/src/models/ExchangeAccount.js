...
exchangeAccountSchema.index({ tenantId: 1, accountNumber: 1 }, { unique: true });
...
exchangeAccountSchema.statics.generateAccountNumber = function(tenantId, currency) {
...
exchangeAccountSchema.statics.getCustomerAccounts = function(tenantId, customerId) {
  return this.find({
    tenantId,
    customerId,
    status: { $ne: 'closed' }
...
exchangeAccountSchema.statics.getByAccountNumber = function(tenantId, accountNumber) {
  return this.findOne({
    tenantId,
    accountNumber
...
exchangeAccountSchema.methods.updateBalance = function(amount, type = 'deposit') {
  if (type === 'deposit') {
    this.balance += amount;
...
exchangeAccountSchema.methods.canWithdraw = function(amount) {
  ...
  if (this.limits.monthlyWithdrawal > 0 && 
