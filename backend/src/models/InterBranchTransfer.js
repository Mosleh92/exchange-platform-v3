...
// Indexes
interBranchTransferSchema.index(
  { tenantId: 1, transferNumber: 1 },
  { unique: true },
);
...
interBranchTransferSchema.statics.getPendingTransfers = function (
  tenantId,
  branchId = null,
) {
  const query = {
...
interBranchTransferSchema.statics.getTransfersByStatus = function (
  tenantId,
  status,
  branchId = null,
) {
...
interBranchTransferSchema.methods.approveTransfer = function (
  approvedBy,
  notes = "",
) {
...
interBranchTransferSchema.methods.rejectTransfer = function (
  rejectedBy,
  reason,
) {
  if (this.status !== "pending") {
...
interBranchTransferSchema.methods.verifyTransfer = function (
  verificationCode,
  verifiedBy,
) {
...
interBranchTransferSchema.methods.completeTransfer = function (
  recipientInfo = {},
) {
...
interBranchTransferSchema.methods.cancelTransfer = function (
  cancelledBy,
  reason,
) {
...
interBranchTransferSchema.pre("save", function (next) {
  ...
  if (this.isNew && !this.security.verificationCode) {
    this.security.verificationCode =
