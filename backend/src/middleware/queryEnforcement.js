...
const FILTERED_METHODS = ['find', 'findOne', 'findOneAndUpdate', 'findOneAndDelete', 'update', 'updateMany', 'deleteOne', 'deleteMany'];
...
class QueryEnforcementMiddleware {
        ...
        FILTERED_METHODS.forEach(method => {
          if (Model[method]) {
            ...
            Model[method] = function(...args) {
              ...
              if (method === 'findOneAndUpdate' || method === 'findOneAndDelete') {
                const filter = args[0] || {};
              ...
              return originalMethods[`${modelName}_${method}`].apply(this, args);
            };
          }
        });
      });
  ...
  static validateTenantAccess(req, res, next) {
    ...
    if (req.user?.tenantAccess && !req.user.tenantAccess.includes(tenantId) && req.user.role !== 'superAdmin') {
