...
exports.createCustomer = async (req, res) => {
  try {
    ...
    console.log(
      `[SMS] Verification code for ${customer.phone}: ${verificationCode}`,
    );
...
exports.createCustomerTransaction = async (req, res) => {
  try {
    const { customerId } = req.params;
    const { type, currency, amount, description, metadata = {} } = req.body;
    ...
    await account.updateBalance(
      currency,
      amount,
      type === "deposit" || type === "transfer_in" || type === "refund"
        ? "add"
        : "subtract",
    );
  ...
  try {
    ...
    if (!customer.verificationCode || !customer.verificationCodeExpires) {
      return res
        ...
        .json({
          success: false,
          message: "کد تأیید برای این مشتری ثبت نشده است",
        });
    }
    if (customer.verificationCodeExpires < new Date()) {
      return res
        .status(400)
        .json({ success: false, message: "کد تأیید منقضی شده است" });
    }
    if (customer.verificationCode !== code) {
      return res
        .status(400)
        .json({ success: false, message: "کد تأیید اشتباه است" });
    }
