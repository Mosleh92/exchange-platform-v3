...
class SubscriptionController {
  ...
  async createSubscription(req, res) {
    try {
      ...
      const subscription = await SubscriptionService.createSubscription({ tenantId, planId, startDate, endDate, autoRenew });
  ...
  async renewSubscription(req, res) {
    try {
      ...
      if (!subscription || subscription._id.toString() !== subscriptionId) {
        return res.status(404).json({ success: false, message: 'اشتراک فعال یافت نشد' });
      }
      // محاسبه تاریخ جدید پایان
      let newEndDate = new Date(subscription.endDate);
      const plan = await Plan.findById(subscription.planId);
      if (plan.duration === 'monthly') newEndDate.setMonth(newEndDate.getMonth() + 1);
      else if (plan.duration === 'yearly') newEndDate.setFullYear(newEndDate.getFullYear() + 1);
      const renewed = await SubscriptionService.renewSubscription({ subscriptionId, newEndDate });
  ...
  async getSubscriptionHistory(req, res) {
    try {
      const history = await SubscriptionService.getSubscriptionHistory(req.user.tenantId);
