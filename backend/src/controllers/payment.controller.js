...
exports.getAllPayments = async (req, res) => {
  try {
    const { page = 1, limit = 20, status, customerId, transactionId } = req.query;
...
exports.uploadReceipt = [
  ...
  async (req, res) => {
    try {
      const { paymentId } = req.params;
      const { description, accountHolderName, paymentDate, referenceNumber, amount } = req.body;
...
exports.getPendingPayments = async (req, res) => {
  try {
    ...
    const payments = await Payment.getPendingPayments(req.tenant?.id || req.user.tenantId, parseInt(days));
