  ...
  try {
    ...
    if (receipt) {
      ...
      await settings.addToHistory(
        "receipt",
        oldReceipt,
        JSON.stringify(settings.receipt),
        req.user.id,
      );
    }
    ...
    if (accounting) {
      ...
      await settings.addToHistory(
        "accounting",
        oldAccounting,
        JSON.stringify(settings.accounting),
        req.user.id,
      );
    }
...
exports.manageReceiptTemplates = async (req, res) => {
  try {
    ...
    switch (action) {
      ...
      case "update": {
        const templateIndex = settings.receipt.templates.findIndex(
      ...
      case "delete": {
        const deleteIndex = settings.receipt.templates.findIndex(
          (t) => t.name === template.name,
        );
        if (deleteIndex === -1) {
          return res.status(404).json({
...
exports.testDeliverySettings = async (req, res) => {
  try {
    ...
    switch (channel) {
      case "email": {
        result = await testEmailDelivery(
          settings.receipt.delivery.email,
          testData,
        );
        break;
      }
      ...
      case "whatsapp": {
        result = await testWhatsAppDelivery(
          settings.receipt.delivery.whatsapp,
          testData,
        );
        break;
      }
...
async function startMigrationProcess(
  ...
  try {
    ...
    console.log(
      `Starting migration for tenant ${tenantId} from ${sourceSystem}`,
    );
