...
exports.createOrder = async (req, res) => {
  try {
    ...
    const order = await CryptoOrder.create({
      ...
      statusHistory: [
        { status: "pending_payment", changedBy: req.user.userId },
      ],
    });
...
exports.cancelOrder = async (req, res) => {
  try {
    ...
    if (order.status !== "pending_payment") {
      return res
        .status(400)
        .json({ success: false, message: "سفارش قابل لغو نیست" });
    }
...
exports.createInPersonOrder = async (req, res) => {
  try {
    ...
    res
      ...
      .json({
        success: false,
        message: "خطا در ثبت معامله حضوری",
        error: err.message,
      });
  }
};
...
exports.markCashPaid = async (req, res) => {
  try {
    ...
    if (order.status !== "pending_payment") {
      return res
        .status(400)
        .json({ success: false, message: "سفارش قابل تغییر نیست" });
    }
    order.status = "paid";
    order.statusHistory.push({
      status: "paid",
      changedBy: req.user.userId,
      notes: "پرداخت نقدی انجام شد",
    });
    ...
    res
      ...
      .json({
        success: false,
        message: "خطا در ثبت پرداخت نقدی",
        error: err.message,
      });
  }
};
...
exports.markCashReceived = async (req, res) => {
  try {
    ...
    res
      ...
      .json({
        success: false,
        message: "خطا در ثبت دریافت نقدی",
        error: err.message,
      });
  }
};
...
exports.markVerified = async (req, res) => {
  try {
    ...
    if (order.status !== "paid") {
      return res
        .status(400)
        .json({ success: false, message: "سفارش باید ابتدا پرداخت شود" });
    }
    ...
    if (order.type === "buy") {
      ...
      if (wallet.balance < order.amount) {
        return res
          .status(400)
          .json({ success: false, message: "موجودی کیف پول کافی نیست" });
      }
      wallet.balance -= order.amount;
      cashAccount.balance -= order.fiatAmount;
      cashAccount.availableBalance -= order.fiatAmount;
    ...
    order.statusHistory.push({
      status: "confirmed",
      changedBy: req.user.userId,
      notes: "تایید نهایی و حسابداری انجام شد",
    });
    order.profit = profit;
    order.fee = fee;
    await order.save();
    ...
    res
      ...
      .json({
        success: false,
        message: "خطا در تایید نهایی و حسابداری معامله",
        error: err.message,
      });
  }
};
...
exports.pendingAlerts = async (req, res) => {
  try {
    ...
    res
      ...
      .json({
        success: false,
        message: "خطا در هشدار معاملات ناقص",
        error: err.message,
      });
  }
};
