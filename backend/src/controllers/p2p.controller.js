  ...
  async recordTransaction(req, res) {
    try {
      ...
      if (announcement.createdBy.toString() === req.user.userId) {
        return res
          .status(400)
          .json({ error: "Cannot trade with your own announcement" });
      }
      ...
      const availableAmount =
        announcement.amount - (totalTraded[0]?.total || 0);
      ...
      res.status(201).json({
        ...
        transaction,
        message:
          "Transaction recorded successfully. Status: Pending verification.",
      });
    } catch (error) {
      res.status(500).json({
