  ...
  try {
    ...
    if (receipt && receipt.pdfFile && receipt.pdfFile.path) {
      ...
      if (isFake) {
        ...
        return res
          .status(400)
          .json({ success: false, message: "رسید جعلی یا دستکاری شده است." });
      }
    }
...
exports.getReceiptById = async (req, res) => {
  try {
    ...
    const receipt = await Receipt.findOne({
      ...
      .populate(
        "transactionId",
        "transactionId type amount currency description",
      )
      .populate("createdBy", "name");
...
exports.downloadReceiptPDF = async (req, res) => {
  try {
    ...
    if (
      !receipt.pdfFile.path ||
      !(await fs
        .access(receipt.pdfFile.path)
        .then(() => true)
        .catch(() => false))
    ) {
      return res.status(404).json({
...
module.exports = {
  ...
  generateReceiptsForExistingTransactions:
    exports.generateReceiptsForExistingTransactions,
};
